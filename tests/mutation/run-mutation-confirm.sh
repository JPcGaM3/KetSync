...
mutant "a writing verb is not recognised as one" \
  's{\Q    migrate|replica|failback|distribute|recall|prepare|isolate|restore|evacuate|cleanup|recover|sync|mail-setup) return 0;;\E}{    nothing) return 0;;}' \
  1

mutant "sync stops counting as something that writes to other machines" \
  's{\Qrestore|evacuate|cleanup|recover|sync|mail-setup) return 0;;\E}{restore|evacuate|cleanup|recover|mail-setup) return 0;;}' \
  12
...
mutant "cleanup is not counted as a verb that writes" \
  's{\Qevacuate|cleanup|recover|sync|mail-setup) return 0;;\E}{evacuate|recover|sync|mail-setup) return 0;;}' \
  16 17
