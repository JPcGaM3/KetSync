...
ROOT="$(cd "$HERE/../.." && pwd)"
SIM="$ROOT/tests/sim/ct-distribute/run-sim-distribute.sh"
SRC="$ROOT/engines/ct-distribute.sh"
...
mutant "the engine reads the copy beside itself instead of ketsync's own table" \
  's{\QNODEMAP="\E\$BASE\Q/nodes.map"    # ip <TAB> pve node name, generated by ketsync\E\n\Qif [[ -f "\E\$BASE\Q/../bin/ketsync" && -f "\E\$BASE\Q/../lib/common.sh" ]]; then\E}{NODEMAP="\$BASE/nodes.map"    # ip <TAB> pve node name, generated by ketsync\nif false; then}' \
  35

mutant "only the node map is taken from ketsync, the placement table is not" \
  's{\Q  [[ -f "\E\$_ks\Q/conf/fleet.tsv" ]] && FLEET="\E\$_ks\Q/conf/fleet.tsv"\E}{  :}' \
  35
...
mutant "CT_LOG is never set, so every line goes to the day log alone" \
  's!\Q  CT_LOG="\E\$LOGDIR\Q/ct/distribute-\E\$_ct\Q-\E\$\Q(date +%F).log"\E!  :!' \
  48
