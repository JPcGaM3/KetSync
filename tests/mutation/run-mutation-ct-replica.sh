...
# which is exactly what it looked like before it existed, so a suite that
# cannot tell is a suite that would have called the old behaviour green.
...
mutant "CT_LOG is never set, so every line goes to the day log alone" \
  's!\Q  CT_LOG="\E\$LOGDIR\Q/ct/replica-\E\$CT\Q-\E\$\Q(date +%F).log"\E!  :!' \
  114 115

mutant "CT_LOG survives the iteration, and the run summary leaks into the last CT's file" \
  's!\Q  CT_LOG=""                     # this container'"'"'s turn is over; the file stays\E!  :!' \
  115
...
mutant "progress prints raw bytes again instead of a size a person reads" \
  's!\Qprogress: \E\$\Q(hsize "\E\$_b\Q")\E!progress: \$_b!' \
  114
