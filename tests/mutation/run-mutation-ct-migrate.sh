...
ROOT="$(cd "$HERE/../.." && pwd)"
SIM="$ROOT/tests/sim/ct-migrate/run-sim-migrate.sh"
SRC="$ROOT/engines/ct-migrate.sh"
...
mutant "CT_LOG is never set, so every line goes to the lane log alone" \
  's!\Q  CT_LOG="\E\$LOGDIR\Q/ct/migrate-\E\$new_ctid\Q-\E\$\Q(date +%F).log"\E!  :!' \
  74

mutant "CT_LOG survives the row, and the run summary leaks into the last CT's file" \
  's!\Q  CT_LOG=""                     # this row'"'"'s turn is over; the file stays\E!  :!' \
  74
