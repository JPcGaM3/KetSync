...
ROOT="$(cd "$HERE/../../.." && pwd)"
ENGINE="${ENGINE:-$ROOT/engines/ct-distribute.sh}"
...
# The layout every real install has: <repo>/engines/. ketsync keeps fleet.tsv
...
if scenario "35: vendored under a ketsync, its tables win over the copies beside the engine"; then
  ...
  printf '# ip\tpve node name\n%s\tpve01\n%s\tpve02\n' "$T1" "$T2" > "$REPO/engines/nodes.map"
  printf '300\t10.100.1.31\t%s\tlocal-lvm\n' "$T1" > "$REPO/conf/fleet.tsv"
  printf '# ip\tpve node name\n%s\tpve01\n%s\tpve02\n' "$T1" "$T2" > "$REPO/conf/nodes.map"
  ENGINE_PATH="$REPO/engines/ct-distribute.sh" run_engine --ctid 300 --list
  rc_is 0; clean
...
if scenario "47: a conf with one broken line refuses the run instead of running on defaults"; then
...
# =============================================================================
if scenario "48: a placement says where it is - one progress line, and one file per CT"; then
  ...
  _cl=( "$WORK"/logs/ct/distribute-300-*.log )
  ...
  grep -qF "placed: CT 9300 on pve01" "${_cl[0]}" 2>/dev/null \
    || _err "the per-CT log is missing the placement verdict"
  # the run summary belongs to the day log alone; a forgotten CT_LOG leaks it
  # into the last CT's file
  grep -qF "distribute finished" "${_cl[0]}" 2>/dev/null \
