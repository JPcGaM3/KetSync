...
ROOT="$(cd "$HERE/../.." && pwd)"
SCRIPT="${SCRIPT:-$ROOT/tools/c2v-inside.sh}"
...
PREP="$ROOT/tools/c2v-prepare.sh"
...
run_famuse(){ # run_famuse <family> <label> <iso>
  : > "$T/log"
  (
    GUEST_FAMILY="$1"; LABEL="$2"; ISO="$3"; GUEST_REL=""; SELF="$ROOT/tools"
    log(){ printf '%s\n' "$*" >> "$T/log"; }
    die(){ printf 'DIE: %s\n' "$*" >> "$T/log"; return 1; }
    # shellcheck disable=SC2317  # called from the sourced block
...
DEB="$ROOT/tools/c2v-inside-deb.sh"
