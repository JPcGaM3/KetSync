...
# logs/ct/ is pruned by the same clock and the same prefix - a per-CT file is
# the same day said again, so it ages out on the same day.
if (( LOG_KEEP_DAYS > 0 )); then
  find "$LOGDIR" "$LOGDIR/ct" -maxdepth 1 -type f -name 'recall-*.log' \
...
do_ct(){   # $1 = production ctid
  ...
  if [[ -t 1 ]]; then
    ...
    rsh "$CT_FROM" "$rscmd --info=progress2 --no-inc-recursive \
      -e 'ssh -o BatchMode=yes -o StrictHostKeyChecking=accept-new' \
      '$mnt/' '$BKP_SSH:$CT_DSTMNT/' 2>/dev/null; echo rc=\$?" | rs_progress "$ct" "$_rsout"
    out=$(cat "$_rsout"); rm -f "$_rsout"
  fi
...
for _ct in "${CTS[@]}"; do
  ...
  CT_LOG="$LOGDIR/ct/recall-$_ct-$(date +%F).log"
  hr_ct
  do_ct "$_ct" || true
  cleanup_ct
