...
#
#  TWO KINDS OF CONNECTION, and both have to exist before you need them:
#
#    the BACKUP node   everything comes through here - the CT configs out of
#                      pmxcfs, the copy's status, the node's own identity, and
#                      the data itself. Nothing works without it.
#    each PROD node    one question only: is this container stopped (B1). It is
#                      asked with `pct status` over ssh, on the node itself, so
#                      it needs no quorum, no pvedaemon proxying and no cluster
#                      membership - which is what keeps this usable against a
#                      standalone node or a node in another cluster. The cluster
#                      API can only answer for a member of its own cluster.
#
#  This host is outside the cluster on purpose, so it has neither the cluster's
#  /etc/hosts nor its DNS. PVE gives us a node NAME (configs live under
#  /etc/pve/nodes/<name>/), and nodes.map turns that name into an address before
#  any ssh is attempted. Without the map the name is used as a hostname, exactly
#  as this engine originally did, and then it does need a resolver.
#
#  BEFORE YOU NEED THIS, run `ct-failback.sh --list` on an ordinary day: it is
#  where you find out a connection is missing, on a Tuesday rather than
#  mid-incident, and it exits 1 rather than 0 while saying so.
#
...
# ip<TAB>pve-node-name, generated by `ketsync doctor` from the cluster and
...
if [[ -f "$CONF" ]]; then
  ...
  _conferr="$(mktemp "${TMPDIR:-/tmp}/ctback-conf.XXXXXX")"
...
# ---------- the exclude list, and it is the SAME one ct-replica.sh reads -----
# Both engines run rsync --delete between the same two rootfs. If a site adds a
# pattern to the replication side only, that path is never copied to the copy -
# and then a failback, which is --delete in the other direction, DELETES it
# from production. One file, read by both, is the only shape of this that
# cannot do that. Refused rather than defaulted, for the same reason as there.
if [[ ! -r "$EXCL" ]]; then
  ...
  echo "ERROR:   This engine reads the SAME file, because it copies the same rootfs in" >&2
  echo "ERROR:   the other direction and also runs --delete. Restore it from the repo," >&2
  echo "ERROR:   or write it back by hand - the shipped contents are three lines:" >&2
  echo "ERROR:     /tmp/*" >&2
  echo "ERROR:     /run/*" >&2
...
# or no row for that node, and the caller decides what to do about it: this
# never guesses an address, because guessing one means asking the WRONG machine
# whether a container is stopped and then writing into the right machine's image.
node_ip(){ awk -v n="$1" '$1!~/^#/ && $2==n{print $1; exit}' "$NODEMAP" 2>/dev/null; }

CT_TGT=""; CT_DEST=""; CT_NODE=""; CT_HOST=""; CT_SID=""; CT_VOL=""; CT_IMG=""
...
probe_ct(){
  ...
  # B1 asks the node itself. `pct status` runs locally there, so it needs no
  # quorum, no pvedaemon proxying and no cluster membership at all - which is
  # what keeps this working against a node that is standalone, in another
  # cluster, or in a cluster that is having a bad day. The cluster API can only
  # answer for a member of the same cluster.
  #
  # PVE hands us a NAME (the config lives at /etc/pve/nodes/<name>/), and this
  # host is outside the cluster with neither its /etc/hosts nor its DNS, so the
  # name is turned into an address here rather than left to a resolver that may
...
failback_one(){
  ...
  # --- B1 ---
  if [[ -z "$CT_PSTAT" ]]; then
    log "[$ct] GUARD B1: cannot reach $CT_NODE (${CT_HOST}) to check the CT - 'unverified' is not 'stopped'"
    log "[$ct] GUARD B1:   writing into the image of a CT that turns out to be running corrupts"
    log "[$ct] GUARD B1:   the LIVE container. Fix ssh to $CT_HOST, then run again."
    return 2
  fi
  if [[ "$CT_PSTAT" != "stopped" ]]; then
    log "[$ct] GUARD B1: production CT is '$CT_PSTAT' on $CT_NODE, must be stopped"
    log "[$ct] GUARD B1:   (Stop it via GUI, or run: ssh root@$CT_HOST pct shutdown $ct)"
    return 2
  fi
  # --- B2, per-CT half ---
  if (( FINAL )); then
...
if (( LIST )); then
  ...
  if (( ${#UNREACHABLE[@]} )); then
    ...
    log "  this host needs root ssh to each of those addresses, before you need it:"
    for u in "${UNREACHABLE[@]}"; do log "    ssh -o BatchMode=yes root@${u#*:} true"; done
    log "  if that says 'could not resolve hostname', that address is a NAME because"
    log "  $NODEMAP has no row for it - run 'ketsync doctor' to rebuild the map;"
    log "  if it asks for a password, ssh-copy-id root@<address> from here."
    exit 1
  fi
  exit 0
fi
